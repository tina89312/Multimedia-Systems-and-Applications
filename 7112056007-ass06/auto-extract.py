...
# 找出對應的game level
def find_game_level(width, height, mine_count):
    game_level = 0

    if width == 9 and height == 9 and mine_count == 10:
        game_level = 1
    elif width == 16 and height == 16 and mine_count == 40:
        game_level = 2
    elif width == 30 and height == 16 and mine_count == 99:
        game_level = 3
    elif width == 30 and height == 24 and mine_count == 120:
        game_level = 4
    elif width == 30 and height == 24 and mine_count == 360:
        game_level = 5
...
# 儲存地雷位置與提取秘密訊息
def save_mine_positions_and_message(locations, width, height, mine_count, cell_size, output_txt):
...
def find_images_with_pattern(pattern):
    files = os.listdir(os.path.join('.', '3-Marke'))
    image_files = [f for f in files if re.match(pattern, f)]
    return image_files

# 主程式
def auto_extract(image_path, mine_template_path, width, height, mine_count, time, game_level, cell_size):
    ...
    cv2.imwrite('5-Recog' + image_path[7:-7] + "Rec_" + image_path[-7:], img_with_mines)
    
    # 儲存地雷位置與秘密訊息
    save_mine_positions_and_message(locations, width, height, mine_count, cell_size, f'6-Extra/Mine{game_level}_Extra_Char_{time}.txt')

# 執行範例
if __name__ == "__main__":
    # 定義圖片名稱格式的正則表達式
    pattern = r'Mine_\d+_\d+_\d+_Mark_\d{3}\.png'
    ...
    for image_file in image_files:
        # 遊戲板的寬、高與地雷數量
        match = re.match(r'Mine_(\d+)_(\d+)_(\d+)_Mark_(\d{3})\.png', image_file)
        width = int(match.group(1))
        height = int(match.group(2))
        mine_count = int(match.group(3))
        time = match.group(4)
        game_level = find_game_level(width, height, mine_count)
        ...
        # 自動提取地雷位置和秘密訊息
        auto_extract(f'3-Marke/{image_file}', mine_template_path, width, height, mine_count, time, game_level, cell_size)
