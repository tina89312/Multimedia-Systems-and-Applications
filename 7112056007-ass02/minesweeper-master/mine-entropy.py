...
def main():
    ...
    if choice == '1':
        ...
        filename = f"../Entropy-{WIDTH:02d}-{HEIGHT:02d}-{MINES:03d}-{SN}.txt"
        ...
        output_filename = f"../Entropy-{WIDTH:02d}-{HEIGHT:02d}-{MINES:03d}-1.txt"
        with open(output_filename, 'w') as f:
            f.write(' '.join(f"{count}" for count in counts) + f" {entropy:.6f}\n")
